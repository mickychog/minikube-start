...
# Acceder
minikube service mi-nginx
```

#### WordPress

```bash
helm install mi-wp bitnami/wordpress \
  --set wordpressUsername=admin \
  --set wordpressPassword=secreto123 \
  --set service.type=NodePort

# Ver contraseña generada automáticamente
kubectl get secret mi-wp-wordpress \
...
# Conectar con redis-cli desde otro Pod
kubectl run redis-cli --rm -it \
  --image=redis \
  -- redis-cli -h mi-redis-master
```
